...
- [x] 7. Voice pipeline — ElevenLabs Scribe STT, ElevenLabs TTS, anxiety/disfluency heuristic.
      Test: `backend/tests/test_voice.py` — passing (5 tests). **Live TTS call surfaced a real account issue** (see Known Issues) — the code path itself is confirmed correct (the error is ElevenLabs rejecting the request, not a bug in the request). STT not live-tested (no real audio sample on hand); wrapper logic is unit-verified against the actual installed SDK's response shape.
...
## Known issues (not code bugs — need a decision from you)

- **ElevenLabs TTS returns HTTP 402** on the configured voice: *"Free users cannot use library voices via the API. Please upgrade your subscription to use this voice."* Your ElevenLabs account is on the free tier, which blocks API access to premade/library voices like `tAtHhBlA3E0eKZJKNSKE`. Options: upgrade the ElevenLabs plan, or point `ELEVENLABS_VOICE_ID` at a voice your account can use via API (e.g. a voice you've personally cloned, which free tier typically does allow for TTS).
