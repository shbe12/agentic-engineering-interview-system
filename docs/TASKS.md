...
**Planning only — not being implemented yet.** This is the task breakdown for `docs/PRD.md` Sprint v2 (video features / communicative & anxiety-pressure patterns), captured for when `/dev` is picked up later. The two Open Decisions in the PRD (analysis approach, video storage) should be confirmed before task 1 starts.
